...
<div class="upload-wrap">
    <div class="upload-card">
        <h2>📤 Upload Hadith</h2>
        <p class="sub">Add a hadith with book, narrator, Arabic text and translations</p>

        @if(session('success'))
            <div class="alert"><i class="fas fa-check-circle"></i> {{ session('success') }}</div>
        @endif

        <form method="POST" action="{{ route('admin.hadith.upload') }}">
            @csrf

            <div class="row-2">
                <div>
                    <label for="book">Book / Collection</label>
                    <input type="text" id="book" name="book" class="form-control" placeholder="e.g. Sahih Bukhari">
                </div>
                <div>
                    <label for="narrator">Narrator</label>
                    <input type="text" id="narrator" name="narrator" class="form-control" placeholder="e.g. Abu Hurairah (RA)">
                </div>
            </div>

            <label for="text_ar">Arabic Text</label>
            <textarea id="text_ar" name="text_ar" class="form-control arabic" placeholder="نص الحديث..."></textarea>

            <label for="text_en">English Translation</label>
            <textarea id="text_en" name="text_en" class="form-control" placeholder="Enter English translation..." required></textarea>

            <label for="text_ur">Urdu Translation</label>
            <textarea id="text_ur" name="text_ur" class="form-control" placeholder="اردو ترجمہ درج کریں..." required style="direction: rtl; text-align: right; font-family: 'Amiri', serif; font-size: 18px;"></textarea>

            <label for="reference">Reference</label>
            <input type="text" id="reference" name="reference" class="form-control" placeholder="e.g. Bukhari 1234">

            <button type="submit" class="btn-submit">
                <i class="fas fa-save"></i> Save Hadith
            </button>
